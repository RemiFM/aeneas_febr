...
def monotype2(loads, cell, V_bus, cycles=[0]):
    ...
    result = {
        ...
        "P_HE": P if cell.dis_rate < 2 else P*0,
        "P_joule_HE": sol.value(P_joule) if cell.dis_rate < 2 else sol.value(P_joule)*0,
        "SOC_HE": sol.value(SOC) if cell.dis_rate < 2 else sol.value(SOC)*0,
        "V_HE": sol.value(V) if cell.dis_rate < 2 else sol.value(V)*0,
        "I_HE": sol.value(I) if cell.dis_rate < 2 else sol.value(I)*0,
        "M_HE": sol.value(M) if cell.dis_rate < 2 else 0,
        "N_HE": sol.value(N) if cell.dis_rate < 2 else 0,
        "P_HE_joule": sol.value(P_joule_aged) if cell.dis_rate < 2 else sol.value(P_joule_aged)*0,
        "SOC_HE_aged": sol.value(SOC_aged) if cell.dis_rate < 2 else sol.value(SOC_aged)*0,
        "V_aged_HE": sol.value(V_aged) if cell.dis_rate < 2 else sol.value(V_aged)*0,
        "I_aged_HE": sol.value(I_aged) if cell.dis_rate < 2 else sol.value(I_aged)*0,
        "E_HE": sol.value(M)*sol.value(N)*cell.energy if cell.dis_rate < 2 else 0,
        "E_aged_HE": sol.value(E_aged) if cell.dis_rate < 2 else 0,
        "I_HE_rated": sol.value(N) * cell.dis_current if cell.dis_rate < 2 else 0,

        "P_HP": P if cell.dis_rate >= 2 else P*0,
        "P_HP_joule": sol.value(P_joule) if cell.dis_rate >= 2 else sol.value(P_joule)*0,
        "SOC_HP": sol.value(SOC) if cell.dis_rate >= 2 else sol.value(SOC)*0,
        "V_HP": sol.value(V) if cell.dis_rate >= 2 else sol.value(V)*0,
        "I_HP": sol.value(I) if cell.dis_rate >= 2 else sol.value(I)*0,
        "M_HP": sol.value(M) if cell.dis_rate >= 2 else 0,
        "N_HP": sol.value(N) if cell.dis_rate >= 2 else 0,
        "P_joule_aged_HP": sol.value(P_joule_aged) if cell.dis_rate >= 2 else 0*sol.value(P_joule_aged),
        "SOC_HP_aged": sol.value(SOC_aged) if cell.dis_rate >= 2 else sol.value(SOC_aged)*0,
        "V_aged_HP": sol.value(V_aged) if cell.dis_rate >= 2 else sol.value(V_aged)*0,
        "I_aged_HP": sol.value(I_aged) if cell.dis_rate >= 2 else sol.value(I_aged)*0,
        "E_HP": sol.value(M)*sol.value(N)*cell.energy if cell.dis_rate >= 2 else 0,
        "E_aged_HP": sol.value(E_aged) if cell.dis_rate >= 2 else 0,
        "I_HP_rated": sol.value(N) * cell.dis_current if cell.dis_rate >= 2 else 0,
    }
...
def monotype_multi(loads, cell, V_bus, cycles=[0]):
    ...
    for i in range(len(loads)):
        ...
        E_loss = E * 0.2 * (cycles[i]/N_cycles) if N_cycles !=0 else 0 # Energy which will be lost due to degradation at EOL (kWh)
