...
def plot_power(dict_results):
    ...
    plt.figure(figsize=(12, 6)) #8 by 3.2 #12, 6

    plt.plot(dict_results["t"]/3600, dict_results["P"]/1e3, label="Power Demand", color=colors[0] if "P_HE" in dict_results else colors[1], linewidth=1.5)
    plt.fill_between(dict_results["t"]/3600, dict_results["P"]/1e3, 0, label=None, color=colors[0], alpha=0.35)

    if "P_HE" in dict_results:
        plt.plot(dict_results["t"]/3600, dict_results["P_HE"]/1e3, label="High Energy Pack", color=colors[2], linewidth=1.5)
        plt.plot(dict_results["t"]/3600, dict_results["P_HP"]/1e3, label="High Power Pack", color=colors[1], linewidth=1.5)

        plt.fill_between(dict_results["t"]/3600, dict_results["P_HE"]/1e3, 0, label=None, color=colors[2], alpha=0.25)
        plt.fill_between(dict_results["t"]/3600, dict_results["P_HP"]/1e3, 0, label=None, color=colors[1], alpha=0.25)
    ...
    plt.legend(loc='upper right')
    plt.xlim(0, max(dict_results["t"]/3600))  # Assuming time is always positive
    plt.ylim(0, 1.1*max(dict_results["P"]/1e3))
