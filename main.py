...
V_bus = 1000 # Nominal pack voltage (V)
loads = [pd.read_csv("loads/paper/boat_imagine.csv")]
cycles = [5000] #10950 & 520
...
# dict_mono_test = monotype.monotype_multi(loads, cell_SSB, V_bus, cycles)
# print(f"{dict_mono_test['time']:,.2f} ms")
# plotting.plot_multiple(dict_mono_test)
# print(f"N: {dict_mono_test['N']}")
# quit()
...
print("\n\n TRESH1 FINISHED \n \n")
print(f"NHE = {dict_treshold['N_HE']}")
print(f"NHP = {dict_treshold['N_HP']}")
#plotting.plot_power(dict_treshold)
...
plotting.plot_power(dict_opti)
plotting.plot_soc(dict_opti)
# plotting.plot_power(dict_opti2)
quit()
